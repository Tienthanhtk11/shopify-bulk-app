(function () {
  ...
  function patchCartAddBody(body, contentType) {
      ...
      if (data) {
        ...
        if (data && Array.isArray(data.items)) {
          data.items = data.items.map(function (item) {
            ...
            var next = Object.assign({}, item, { quantity: qWant });
            if (planId != null && next.selling_plan == null) {
        ...
        if (data && data.id != null && data.quantity != null) {
          var nextOne = Object.assign({}, data, { quantity: qWant });
          if (planId != null && nextOne.selling_plan == null) {
    ...
    if (
      ...
      try {
        ...
        if (planId != null && !params.get("selling_plan")) {
    ...
    if (typeof FormData !== "undefined" && body instanceof FormData) {
      ...
      if (planId != null && !body.get("selling_plan")) {
  ...
  function buildHtml(d) {
    ...
    var subscriptionCards = allPlans.length
      ? allPlans
          .map(function (plan) {
            var planPricing = calcSubscriptionPrice(
              plan.id,
              d.unitOnetime,
              d.totalOnetime,
              planBaseCompare,
            ...
            return optionCard(
              "subscribe",
              planTitle(plan),
              formatMoney(planPricing.unitSub, cfg.currency),
              buildPlanBadgeText(
                d.unitOnetime,
                planPricing.unitSub,
                planPricing.subscriptionFromShopify,
              ),
              plan.id,
            );
          })
    ...
    var bulkTableBlock = hasBulkTiers
      ? '<div class="subbulk__table-wrap"><table class="subbulk__table" aria-label="Bulk pricing">' +
        ...
        tableRows +
        "</tbody></table></div>"
      : '<p class="subbulk__no-bulk-note">Chưa cấu hình bulk pricing — đang dùng giá sản phẩm theo variant.</p>';
    ...
    return (
      ...
      "</div>" +
      (isSub && activePlan
        ? '<div class="subbulk__details-row">⟳ ' +
          escapeHtml(
            (cfg.planSelectorLabel || "Deliver every") + ": " + planTitle(activePlan),
          ) +
          "</div>"
        : "") +
  ...
  function wire(root) {
    root.querySelectorAll("[data-subbulk-mode]").forEach(function (lbl) {
      lbl.addEventListener("click", function () {
        ...
        var planId = lbl.getAttribute("data-subbulk-plan-id");
        state.purchaseMode = m === "subscribe" ? "subscribe" : "onetime";
        if (state.purchaseMode === "subscribe" && planId) {
